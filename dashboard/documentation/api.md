...
| `GET` | `/api/stats` | `capas.` | - | Public crowd results: every voted cover and the latest day's winner. Reads `analytics_covers`, joining `covers` only for image URLs. The model's verdicts are `/api/detector` |
...
| `GET` | `/api/rag-candidates?limit=&needs=&date=` | `capas.` | Bearer | Up to N classifiable covers (id, newspaper, date, r2_key, url, headlines), newest first, for `scripts/rag_classify.py`. Classifiable means no `ai_club`, a crowd vote, and stored titles unless dated before today. `needs=matches` returns the retrieval backlog instead, which has no such preconditions. Self-converging: repeated calls work through the backlog. See [AI Detector](#ai-detector) |
| `POST` | `/api/reclassify-rag` | `capas.` | Bearer | Classify one cover `{cover_id, r2_key, few_shot, rag_cover_ids, rag_sources}` with an externally-computed RAG few-shot block; the one Llama4 call for that cover. Stores `rag_cover_ids` as `ai_rag_covers`, for provenance. See [RAG](#rag) |
| `POST` | `/api/label-consensus` | `capas.` | Bearer | Record a label the RAG neighbours agreed on, `{cover_id, club, agreed, of, rag_cover_ids, rag_sources}`, with no model call. Re-checks the threshold server-side and writes `ai_source = 'consensus'`. See [RAG](#rag) |
| `POST` | `/api/rag-matches` | `capas.` | Bearer | Record which covers retrieval matched, `{cover_id, rag_cover_ids, rag_sources}`, without classifying: writes `ai_rag_covers` and `ai_rag_source`, no label and no model call. What `rag_classify.py --matches-only` posts to. See [RAG](#rag) |
| `GET` | `/api/detector` | `capas.` | - | The AI Detector's verdicts: every classified cover with the club to display, the model's own answer, the crowd's, and the `others` gate's probabilities, plus the latest day's verdict and archive-wide agreement. `?threshold=` re-resolves the gate for that response only |
| `GET` | `/api/similarities` | `capas.` | - | Every cover with `ai_rag_covers` recorded, plus the covers those ids resolve to, each tagged with the channel that found it (`via`: `headline` or `layout`). Powers `/similarities` and its channel filter, not restricted to voted covers like `/api/stats` |
| `GET` | `/api/vectorize-candidates?limit=&index=` | `capas.` | Bearer | Up to N voted covers still missing that index's vector, newest first. `index=image` (default) returns `url` for `build_vectorize_index.py`; `index=headline` returns `headlines` for `build_headline_index.py` and skips covers with none. Self-converging like `/rag-candidates`. See [Image Embeddings](#image-embeddings) and [Headline Embeddings](#headline-embeddings) |
| `POST` | `/api/vectorize-mark` | `capas.` | Bearer | Set that index's progress column to now for `{cover_ids: [...], index?}` — `vectorized_at` for `image` (default), `headline_vectorized_at` for `headline`. Called after those ids upsert successfully into Vectorize, never before. See [Image Embeddings](#image-embeddings) and [Headline Embeddings](#headline-embeddings) |
