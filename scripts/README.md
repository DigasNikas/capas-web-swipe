...
| `rag_classify.py` | Embed a cover twice — image against `capas-cover-embeddings`, lead headline against `capas-headline-embeddings` — merge the two result sets and fold their labels into the Llama4 prompt as few-shot context. Live mode writes to D1 via the Worker's admin API; `--eval` scores against crowd labels without touching the Worker (pulling the scraped headline text it needs for prompt parity from `/headlines`) | Classify Covers (automatic + manual) |
