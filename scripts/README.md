...
| `rag_classify.py` | Embed a cover, query Vectorize for similar labelled covers, fold their labels into the Llama4 prompt as few-shot context. Live mode writes to D1 via the Worker's admin API; `--eval` scores against crowd labels without touching the Worker (pulling the scraped headline text it needs for prompt parity from `/stats?headlines=1`) | Classify Covers (automatic + manual) |
