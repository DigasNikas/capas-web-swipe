...
| `train_classic_classifier.py` | Classic-ML exercise: flatten covers to pixel vectors, fit 6 classic scikit-learn models + one from-scratch PyTorch MLP on the crowd's own labels, `--split chronological/stratified` and `--per-newspaper` toggles, report accuracy/precision/recall per model — not a production model, no GitHub Action | — |
