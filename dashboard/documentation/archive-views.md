...
One 3px stripe per day, coloured by the club of that day's cover, one row per paper — a whole season of the Portuguese sports press readable in one glance. Pure frontend: `renderBarcode()` in `dashboard.js` reuses the `days` array the calendar already builds, so it follows the época dropdown and adds no request, no endpoint and no asset. It scrolls sideways rather than dropping days; the paper labels are `position: sticky` so they survive the scroll.
...
Runs locally (it pulls ~1250 JPEGs, ~90s on 12 threads), never in the Worker. Output is committed to `dashboard/avg/` — 15 JPEGs plus `counts.json` — so the page just fetches static files, and the section stays hidden if they're absent.
