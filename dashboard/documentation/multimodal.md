...
Covers are called by the largest photo on the page and everything printed with it, not kit colours: at 220px the Portuguese headline is unreadable, so the classifier fetches the full-res original from R2 instead of the thumbnail the rest of the site uses. The prompt asks the model to find that photo first, name its club, then read only its headline — a smaller headline strip near the top of the page used to out-rank a much larger photo lower down, so a cover playing up one club's league position in three words at the top while giving the rest of the page to another club's transfer story got called for the strip instead of the story the page actually leads with. A separate miss showed the fix needed one more piece: a headline can name no club at all ("PRESSA DE VENCER" over a huge Porto photo), and the model filled the gap by reading a small BENFICA rail box it had been told to ignore, rather than the score line printed right under the photo ("AC. VISEU 0 FC PORTO 3") that already named the club. The prompt now says explicitly that a score line, caption or headline printed with the largest photo all belong to it and can be used together — a rail box still doesn't count, because it isn't printed with the photo. The reply itself still asks for the headline before the answer, prose parsed down to one of four keys, headline-first rather than answer-first. No `ANSWER:` line means no label; the parser never guesses.
