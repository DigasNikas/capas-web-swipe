...
Covers are called by the largest photo on the page and that photo's own headline, not kit colours: at 220px the Portuguese headline is unreadable, so the classifier fetches the full-res original from R2 instead of the thumbnail the rest of the site uses. The prompt asks the model to find that photo first, name its club, then read only its headline — a smaller headline strip near the top of the page used to out-rank a much larger photo lower down, so a cover playing up one club's league position in three words at the top while giving the rest of the page to another club's transfer story got called for the strip instead of the story the page actually leads with. The reply itself still asks for the headline before the answer, prose parsed down to one of four keys, headline-first rather than answer-first. No `ANSWER:` line means no label; the parser never guesses.

A club crest is the clearest signal for whose story a block of text is, so the prompt calls it out explicitly, both ways: a crest printed on or beside the dominant photo's own headline names that photo's club (Record in particular runs a crest on a bold top line for each of its teaser boxes), and the same crest on one of the small SPORTING/FC PORTO/BENFICA rail boxes is exactly what marks that box as a separate story rather than something to fold into the dominant one — the crest doesn't override the photo-size rule, it explains why the rule holds.
