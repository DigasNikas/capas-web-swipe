...
| `POST` | `/api/label-consensus` | `capas.` | Bearer | Record a label the RAG neighbours agreed on, `{cover_id, club, agreed, of, rag_cover_ids}`, with no model call. Re-checks the threshold server-side and writes `ai_source = 'consensus'`. See [RAG](#rag) |
