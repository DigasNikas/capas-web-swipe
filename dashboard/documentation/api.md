...
| `GET` | `/api/rag-candidates?limit=` | `capas.` | Bearer | Up to N covers still missing `ai_club` (id, newspaper, date, r2_key, url, headlines), newest first, for `scripts/rag_classify.py` to embed. `headlines` and `date` are what the text channel needs: one to embed, one to drop same-day siblings. Self-converging: repeated calls work through the backlog rather than reprocessing the same covers. See [RAG](#rag) |
